...
export function TeamProfileClient({ initialData, userId }: { initialData: UserData | null; userId: string }) {
  const router = useRouter();
  const [user, setUser] = React.useState<UserData | null>(initialData);
  const [editing, setEditing] = React.useState(false);
  const [saving, setSaving] = React.useState(false);
  const [uploading, setUploading] = React.useState(false);
  const [form, setForm] = React.useState<UserData>(initialData || {});
