...
function buildColumns(data: UserRecord[]): ColumnDef<UserRecord>[] {
...
function CellRenderer({ value }: { value: unknown }) {
  ...
  if (typeof value === "string") {
    ...
    if (value.length > 80) {
      return <span title={value} className="cursor-help">{value.slice(0, 80)}…</span>;
    }
    return <span>{value}</span>;
  }
  if (Array.isArray(value)) {
    return <Badge variant="outline" className="text-muted-foreground">{value.length} items</Badge>;
  }
  ...
  const fetchTeam = React.useCallback(async () => {
    ...
    try {
      ...
      if (json.success) {
        ...
        setError(json.message || "Failed to fetch team");
      }
    } catch (err) {
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-2 py-24">
        <p className="text-muted-foreground text-sm">
          No team members found in <code className="text-xs">Nashville_Users</code>.
        </p>
        <Button variant="outline" size="sm" onClick={fetchTeam}>
    ...
    <div className="flex w-full flex-col gap-4">
      ...
      <div className="flex items-center justify-between px-4 lg:px-6">
        <div className="text-muted-foreground hidden flex-1 text-sm lg:flex">
          {table.getFilteredRowModel().rows.length} total member(s)
        </div>
        <div className="flex w-full items-center gap-8 lg:w-fit">
          <div className="hidden items-center gap-2 lg:flex">
            <Label htmlFor="team-rows-per-page" className="text-sm font-medium">Rows per page</Label>
            <Select value={`${table.getState().pagination.pageSize}`} onValueChange={(value) => table.setPageSize(Number(value))}>
              <SelectTrigger size="sm" className="w-20" id="team-rows-per-page">
